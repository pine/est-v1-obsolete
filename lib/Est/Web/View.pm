...
use Est::Web::ViewFunctions;

# setup view class
sub make_instance {
    my ($class, $context) = @_;
    Carp::croak("Usage: Est::Web::View->make_instance(\$context_class)") if @_!=2;
    ...
    unless (exists $view_conf->{path}) {
        ...
        if ( -d $tmpl_path ) {
            ...
            my $share_tmpl_path = eval { File::Spec->catdir(File::ShareDir::dist_dir('Est'), 'tmpl') };
            if ($share_tmpl_path) {
